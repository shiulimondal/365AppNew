...
const ForeclosureRecords = ({ foreclosureRecords, openLock }) => {
    ...
    const normalForeclosureRecords =
        typeof foreclosureRecords === 'string'
            ? foreclosureRecords
            : Array.isArray(foreclosureRecords)
                ? foreclosureRecords.flatMap(record => record || [])
                : [];
    ...
    return (
        <View>
              {typeof normalForeclosureRecords === 'string' ? (
                ...
                <View style={styles.hide_view}>
                    ...
                    <Text style={{ ...styles.phone_number, color: colors.secondaryFontColor }}>Foreclosure Records: ({workplaceRecords?.length})</Text>
                        {openLock ? (
            ...
            {showHiddenCard && (
                <View style={styles.full_card}>
                    {!showContent ? (
                        ...
                        <>
                            {(() => {
                                const isArray = Array.isArray(normalForeclosureRecords);
                                const isObjectArray = isArray && typeof normalForeclosureRecords[0] === 'object';
                                const isString = typeof normalForeclosureRecords === 'string';

                                if (isString) {
                                    return (
                                        <View>
                                            <Text style={{ ...styles.message_txt, color: colors.secondaryFontColor }}>
                                                {normalForeclosureRecords}
                                            </Text>
                                        </View>
                                    );
                                }

                                if (isArray && typeof normalForeclosureRecords[0] === 'object') {
                                    return normalForeclosureRecords.map((item, index) => (
                                        <View key={index}
                                            style={{
                                                ...styles.hiddenCard,
...
export default ForeclosureRecords
